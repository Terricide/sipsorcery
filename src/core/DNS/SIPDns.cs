...
namespace SIPSorcery.SIP
{
    ...
    public class SIPDns
    {
        ...
        {
            if (uri == null || StringExtensions.IsNullOrWhiteSpace(uri.MAddrOrHostAddress))
            {
                ...
                switch (queryType)
                {
                    case QueryType.SRV:
                        ...
                        try
                        {
                            ...
                            var hostPort = GetHostAndPortFromSrvResult(srvResult);
                            if (hostPort.Host != null)
                            {
                                host = hostPort.Host;
                                port = hostPort.Port != 0 ? hostPort.Port : port;
                            }
                        }
        ...
        private static HostPort GetHostAndPortFromSrvResult(IDnsQueryResponse srvResult)
        {
            // TODO: Re-enable when DnsClient adds cache querying.
            //if (srvResult != null)
            //{
        ...
        /// <returns>The hostname and port for the chosen SRV result record.</returns>
        private static HostPort GetHostAndPortFromSrvResult(ServiceHostEntry[] serviceHostEntries)
        {
